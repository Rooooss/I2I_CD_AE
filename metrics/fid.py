...
inception_images = tf.placeholder(tf.float64, [BATCH_SIZE, 3, None, None])
...
def get_inception_activations(inps):
    ...
    act = np.zeros([n_batches * BATCH_SIZE, 2048], dtype=np.float64)
...
real_Images = [Image.open(file) for file in glob.glob('C:/Users/Joohong/Desktop/2019/CycleGAN-tensorflow/datasets/cityscapes/testB/*jpg')]
val_Images = [Image.open(file) for file in glob.glob('C:/Users/Joohong/Desktop/2019/experiment_collection/correct_load/cityscapes/ours-conv3conv3/test/AtoB/*jpg')]

if __name__ == '__main__':

    realImages = []
    valImages = []
    data_len = len(val_Images)

    for i in range(data_len):
        realImages.append(np.array(real_Images[i].resize((256, 256))))

    for i in range(data_len):
        valImages.append(np.array(val_Images[i]))

    realImages = np.array(realImages)
    valImages = np.array(valImages)

    realImages = np.transpose(realImages, (0, 3, 1, 2))
    valImages = np.transpose(valImages, (0, 3, 1, 2))

    fid_score = get_fid(realImages, valImages)
