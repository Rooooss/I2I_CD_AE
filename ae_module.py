...
def generator_resnet(image, options, reuse=False, name="generator"):
    ...
    with tf.variable_scope(name):
        ...
        def mapping(x, dim, ks=3, s=1, name='map'):
            p = int((ks - 1) / 2)
            y = tf.pad(x, [[0, 0], [p, p], [p, p], [0, 0]], "REFLECT")
            y = instance_norm(conv2d(x, dim, ks, s, padding='VALID', name=name+'_c'))
            y = tf.nn.relu(y)
            # y = tf.pad(tf.nn.relu(y), [[0, 0], [p, p], [p, p], [0, 0]], "REFLECT")
            # y = instance_norm(depthwiseconv2d(y, filters=[ks, ks, 256, 1], padding='VALID', name=name+'_c2'), name+'_bn2')
            return y + x

        def depthwise_residual_block(x, dim, ks=7, s=1, name='res'):
            p = int((ks - 1) / 2)
            y = tf.pad(x, [[0, 0], [p, p], [p, p], [0, 0]], "REFLECT")

            y = instance_norm(depthwiseconv2d(y, filters=[ks, ks, 256, 1], padding='VALID', name=name+'_c1'), name+'_bn1')
            #y = tf.pad(tf.nn.relu(y), [[0, 0], [p, p], [p, p], [0, 0]], "REFLECT")
            #y = instance_norm(depthwiseconv2d(y, filters=[ks, ks, 256, 1], padding='VALID', name=name+'_c2'), name+'_bn2')
            return y + x
        ...
        # r6 - r10, d1 - d2 and pred are decoder of generator networks
        r6 = residule_block(r5, options.gf_dim*4, name='g_r6')
        r7 = residule_block(r6, options.gf_dim*4, name='g_r7')
        r8 = residule_block(r7, options.gf_dim*4, name='g_r8')
        r9 = residule_block(r8, options.gf_dim*4, name='g_r9')
        r10 = residule_block(r9, options.gf_dim * 4, name='g_r10')
        d1 = deconv2d(r10, options.gf_dim*2, 3, 2, name='g_d1_dc')
