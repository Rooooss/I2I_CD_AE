...
def generator_unet(image, options, reuse=False, name="generator"):
    ...
    with tf.variable_scope(name):
        ...
        '''
        #for concatenation
        temp_o_s1 = [options.batch_size, s128, s128, self.gf_dim * 8]
        temp_d1_w = tf.get_variable('temp_d1_w', [4, 4, temp_o_s1[-1], e8_for_concat.get_shape()[-1]],
...
def generator_resnet(image, options, reuse=False, name="generator"):
    ...
    with tf.variable_scope(name):
        ...
        def depthwise_residual_block(x, dim, ks=1, s=1, name='res'):
            p = int((ks - 1) / 2)
            y = tf.pad(x, [[0, 0], [p, p], [p, p], [0, 0]], "REFLECT")
